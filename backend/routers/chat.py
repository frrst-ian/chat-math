...
def _render_and_save(animation_id: int, script: str, db: Session):
    ...
    if not animation:
        return
    try:
        local_path = run_manim_script(script)
        if not local_path:
            animation.status = AnimationStatus.failed
        else:
            public_id = f"chatmath/{uuid.uuid4().hex}"
            stored = save_video(local_path, public_id)
            animation.video_url = stored["url"]
            animation.cloudinary_public_id = stored["public_id"]
            animation.status = AnimationStatus.rendered
...
def chat(request: ChatRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    return ChatResponse(reply=llm_result["reply"], conversation_id=conversation.id, animation_id=animation_id)
